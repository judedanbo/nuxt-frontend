...
const classificationListPage = ref(1)
...
const { data: classification, pending, error, refresh} = useLazyFetch<ClassificationResponse>(config.public.API_BASE_URL +'/inventory/classification/',{
  query: {search: searchClassification},
  watch: [searchClassification]
})

const classificationCreated = () => {
  toast.add({title: 'New classification has been added'})
...
const classificationToEdit = ref()
const openEditForm = (row: Classification) => {
  classificationToEdit.value = row
...
const action = (row: Classification) => [
  [{
    ...
    click: () => navigateTo('/inventory/classification/' + row.id)
  }], [{
    label: 'Delete',
...
<template>
  <section>
    ...
    <UPageHeader headline="Inventory" title="Classification" description="List of product classifications" />
    <div class="flex px-3 py-3.5 border-b border-gray-200 dark:border-gray-700 justify-between">
      <UInput v-model="searchClassification" placeholder="search classification..." autofocus />
      <UButton icon="i-mdi-storefront-plus-outline" label="Add classification" @click="addClassification" />
    </div>
    ...
    <div v-else>
      ...
      <UPageCard class="flex flex-wrap gap-10 mt-5">
        <UTable :rows="classification?.data" :columns="columns">
          <template #name-data="{row}">
            <nuxt-link :to="'/inventory/classification/'+row.id" class="prose prose-md dark:prose-invert font-medium">
              {{ row.name }}
            </nuxt-link>
          </template>
        ...
        <UPagination v-model="classificationListPage" :total="classification?.total_items" :ui="{ rounded: 'first-of-type:rounded-s-md last-of-type:rounded-e-md' }">
    ...
    <USlideover v-model="showCreateForm">
      <form-shell title="Create new Classification">
        <inventory-classification-add @created="classificationCreated" />
      </form-shell>
    </USlideover>
    <USlideover v-model="showEditForm">
      <form-shell title="Edit Classification">
        <inventory-classification-edit :classification="classificationToEdit" @updated="classificationUpdated" />
      </form-shell>
    </USlideover>
  </section>
</template>