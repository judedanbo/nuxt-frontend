...
const { data: brand, error, pending, refresh } = useBrand(id)
const breadcrumb = [{
  label: 'Home',
...
const openEditForm = () => {
  ...
  const deleteBrand = async() => {
    const { data } = await useDeleteBrand(id)
    toast.add({ title: 'Brand deleted successfully' })
    navigateTo('/inventory/brands')
  }
const brandUpdated = () => {
...
<template>
  <section>
    ...
    <div v-else>
      ...
      />
      <UPageCard title="Brand Products" description="Products this brand is associated with" class="mt-5" >
        <UTable :rows="brand?.products" :columns="productTable">
    ...
    <USlideover v-model="showSlideover" :overlay="true" :close-button="true" :size="'2xl'">
      <form-shell title="Edit Brand">
        <inventory-brands-edit :brand="brand" @updated="brandUpdated" />
      </form-shell>
    </USlideover>
    <UModal v-model="showModal" :overlay="true" :close-button="true" :size="'2xl'">
      <div class="p-10 flex flex-col gap-y-4">
        ...
        <div class="flex justify-between gap-2 mt-10">
          ...
          <UButton color="red" variant="solid" @click="deleteBrand">
            Delete
          </UButton>
        </div>
      </div>
