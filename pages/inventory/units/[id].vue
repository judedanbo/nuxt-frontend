...
const { data: unit, error, pending, refresh } = useUnit(id)
const breadcrumb = [{
  label: 'Home',
...
const openEditForm = () => {
  ...
  const deleteUnit = async() => {
    const { data } = await useDeleteUnit(id)
    toast.add({ title: 'Unit deleted successfully' })
    navigateTo('/inventory/units')
  }
const unitUpdated = () => {
...
<template>
  <section>
    ...
    <div v-else>
      ...
      />
      <UPageCard title="Unit Products" description="Products this unit is associated with" class="mt-5" >
        <UTable :rows="unit?.products" :columns="productTable">
    ...
    <USlideover v-model="showSlideover" :overlay="true" :close-button="true" :size="'2xl'">
      <form-shell title="Edit Unit">
        <inventory-units-edit :unit="unit" @updated="unitUpdated" />
      </form-shell>
    </USlideover>
    <UModal v-model="showModal" :overlay="true" :close-button="true" :size="'2xl'">
      <div class="p-10 flex flex-col gap-y-4">
        ...
        <div class="flex justify-between gap-2 mt-10">
          ...
          <UButton color="red" variant="solid" @click="deleteUnit">
            Delete
          </UButton>
        </div>
      </div>
