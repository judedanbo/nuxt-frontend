...
const { data: category, error, pending, refresh } = useCategory(id)
const breadcrumb = [{
  label: 'Home',
...
const openEditForm = () => {
  ...
  const deleteCategory = async() => {
    const { data } = await useDeleteCategory(id)
    toast.add({ title: 'Category deleted successfully' })
...
<template>
  <section>
    ...
    <div v-else>
      ...
      />
      <UPageCard title="Category Products" description="Products this category is associated with" class="mt-5" >
        <UTable :rows="category?.products" :columns="productTable">
    ...
    <USlideover v-model="showSlideover" :overlay="true" :close-button="true" :size="'2xl'">
      <form-shell title="Edit Category">
        <inventory-category-edit :category="category" @updated="categoryUpdated" />
      </form-shell>
    </USlideover>
    <UModal v-model="showModal" :overlay="true" :close-button="true" :size="'2xl'">
      <div class="p-10 flex flex-col gap-y-4">
        ...
        <div class="flex justify-between gap-2 mt-10">
          ...
          <UButton color="red" variant="solid" @click="deleteCategory">
            Delete
          </UButton>
        </div>
      </div>
