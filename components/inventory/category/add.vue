...
const categoryForm = ref({ name: props.value })
const handleSubmit = async () => {
  return  useLazyFetch(config.public.API_BASE_URL +'/inventory/categories/', {
    method: 'POST',
    body: categoryForm,
    onRequest(){      
...
// const handleSubmit = () => {
//   //  const {data: category error, pending} =  submitForm()
//   //  if(error)
// }
</script>
...
<template>
  <UForm class="space-y-4 mt-10" @submit="handleSubmit">
    <UFormGroup label="Category Name " name="name">
      <UInput v-model="categoryForm.name" required/>
